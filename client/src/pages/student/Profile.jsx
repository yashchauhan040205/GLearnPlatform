...
const AVATAR_COLORS = ['bg-primary-600', 'bg-rose-600', 'bg-teal-600', 'bg-violet-600', 'bg-blue-600']
    ...
    <div className="max-w-3xl mx-auto space-y-5 animate-in">
      ...
      <div className="card p-5">
        <div className="flex items-center gap-5">
          <div className="relative">
            ...
            <button className="absolute -bottom-1 -right-1 bg-dark-900 border border-dark-800 rounded-lg p-1 hover:bg-dark-950 transition-colors">
              <Camera className="w-3.5 h-3.5 text-dark-400" />
            </button>
          </div>
          <div className="flex-1">
            <h1 className="text-xl font-bold text-dark-100">{user?.name}</h1>
            ...
            <div className="mt-2 flex gap-4 text-sm">
              <span className="text-dark-400"><span className="text-dark-100 font-semibold">{user?.points}</span> points</span>
              <span className="text-dark-400"><span className="text-dark-100 font-semibold">{user?.streak?.current || 0}</span> day streak</span>
              <span className="text-dark-400"><span className="text-dark-100 font-semibold">{user?.badges?.length || 0}</span> badges</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="flex gap-2 border-b border-dark-700 pb-0">
        {[{ key: 'profile', icon: User, label: 'Profile' }, { key: 'security', icon: Shield, label: 'Security' }, { key: 'preferences', icon: Palette, label: 'Preferences' }].map(({ key, icon: Icon, label }) => (
          <button key={key} onClick={() => setTab(key)} className={`flex items-center gap-2 px-4 py-3 text-sm font-medium border-b-2 -mb-px transition-colors ${tab === key ? 'border-primary-500 text-dark-100' : 'border-transparent text-dark-400 hover:text-dark-100'}`}>
      ...
      {tab === 'security' && (
        <div className="card p-5 space-y-5">
          <h3 className="text-dark-100 font-semibold">Change Password</h3>
