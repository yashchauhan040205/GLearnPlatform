  ...
  return (
    <div className="space-y-6 animate-in">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-black text-white flex items-center gap-2"><BookOpen className="text-indigo-400" />Manage Courses</h1>
          <p className="text-gray-400 mt-1">{total} total courses · {courses.length} displayed</p>
        </div>
          ...
          <div className="flex gap-2">
            {[{ key: 'all', label: 'All' }, { key: 'published', label: '📤 Published' }, { key: 'draft', label: '📝 Draft' }].map(({ key, label }) => (
              <button
                ...
                className={`px-3 py-2 rounded-xl text-xs font-medium transition-all flex-1 ${filter === key ? 'bg-indigo-600 text-white' : 'bg-gray-900 text-gray-400 border border-gray-800 hover:text-white'}`}
              >
                {label}
              </button>
            ))}
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-gray-700">
                <th className="text-left text-gray-400 text-xs font-semibold uppercase tracking-wider px-5 py-4">Course</th>
                <th className="text-left text-gray-400 text-xs font-semibold uppercase tracking-wider px-5 py-4 hidden md:table-cell">Educator</th>
                <th className="text-left text-gray-400 text-xs font-semibold uppercase tracking-wider px-5 py-4 hidden lg:table-cell">Stats</th>
                <th className="text-left text-gray-400 text-xs font-semibold uppercase tracking-wider px-5 py-4">Status</th>
                <th className="text-right text-gray-400 text-xs font-semibold uppercase tracking-wider px-5 py-4">Actions</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-gray-800">
              {loading ? (
                [...Array(8)].map((_, i) => <tr key={i}><td colSpan={5} className="px-5 py-4"><div className="h-10 bg-gray-900 rounded animate-pulse" /></td></tr>)
              ) : courses.length === 0 ? (
                    ...
                    <div className="flex items-center gap-3">
                      <div className="w-10 h-10 rounded-lg overflow-hidden shrink-0">
                        {course.thumbnail ? <img src={course.thumbnail} className="w-full h-full object-cover" alt="" /> : (
                          <div className="w-full h-full bg-indigo-600/20 flex items-center justify-center"><BookOpen className="w-4 h-4 text-indigo-400" /></div>
                        )}
                      </div>
                      <div>
                  ...
                  <td className="px-5 py-4 hidden lg:table-cell">
                    <div className="flex items-center gap-4 text-xs text-gray-400">
                      <div className="flex items-center gap-1">
                        ...
                        <span className="text-gray-200 font-medium">{course.enrolledStudents?.length || 0}</span>
                      </div>
                      <div className="flex items-center gap-1">
                        <Star className="w-3 h-3 fill-yellow-400 text-yellow-400" />
                        <span className="text-gray-200 font-medium">{typeof course.rating === 'number' && course.rating > 0 ? course.rating.toFixed(1) : '—'}</span>
                      </div>
                    </div>
                  </td>
                  <td className="px-5 py-4">
                    <button onClick={() => handleTogglePublish(course._id, course.isPublished)}
                      className={`flex items-center gap-1.5 text-xs px-2.5 py-1.5 rounded-full transition-colors ${course.isPublished ? 'bg-green-600/20 text-green-400 hover:bg-red-600/20 hover:text-red-400' : 'bg-gray-700 text-gray-400 hover:bg-green-600/20 hover:text-green-400'}`}>
                      {course.isPublished ? <><Eye className="w-3 h-3" />Published</> : <><EyeOff className="w-3 h-3" />Draft</>}
                    </button>
                  </td>
                  <td className="px-5 py-4">
                    <div className="flex items-center justify-end gap-1">
                      <button onClick={() => navigate(`/admin/courses/${course._id}/edit`)} className="p-1.5 rounded-lg text-gray-400 hover:text-indigo-400 hover:bg-indigo-400/10 transition-colors" title="Edit course">
                        <Edit className="w-4 h-4" />
                      </button>
                      <button onClick={() => handleDelete(course._id, course.title)} className="p-1.5 rounded-lg text-gray-400 hover:text-red-400 hover:bg-red-400/10 transition-colors" title="Delete course">
