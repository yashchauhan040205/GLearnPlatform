...
const Register = () => {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-b from-white via-emerald-50/60 to-white px-4 py-8">
          ...
          <div className="grid grid-cols-2 gap-3 mb-5">
            {[{ value: 'student', icon: BookOpen, label: 'Student', desc: 'Learn & earn XP' }, { value: 'educator', icon: GraduationCap, label: 'Educator', desc: 'Create & teach' }].map(({ value, icon: Icon, label, desc }) => (
              <button key={value} type="button" onClick={() => setForm(p => ({ ...p, role: value }))}
                className={`p-3 rounded-lg border transition-colors text-left ${form.role === value ? 'border-emerald-500 bg-emerald-50' : 'border-emerald-100 hover:border-emerald-300'}`}>
                <Icon size={18} className={form.role === value ? 'text-emerald-600 mb-1' : 'text-slate-400 mb-1'} />
                <p className={`font-medium text-sm ${form.role === value ? 'text-slate-900' : 'text-slate-600'}`}>{label}</p>
