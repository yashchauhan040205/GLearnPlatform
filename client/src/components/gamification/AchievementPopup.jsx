  ...
  return (
    <div className="fixed bottom-6 right-6 z-50 w-72 animate-in">
      <div className={`${style.bg} border ${style.border} rounded-lg p-4 shadow-lg`}>
        <button onClick={onClose} className="absolute top-2 right-2 text-dark-500 hover:text-dark-300 transition-colors">
          <X size={14} />
        </button>

        <div className="text-center">
          <div className="text-4xl mb-2 inline-block">{badge?.icon || '🏆'}</div>
          <div className="text-xs font-semibold uppercase tracking-wider text-dark-400 mb-1">Achievement Unlocked</div>
          ...
          <div className="flex items-center justify-center gap-3 text-xs">
            <span className="text-primary-600">+{badge?.xpReward} XP</span>
